...
- Полагаем, что оба числа либо являются отрицательными, либо положительными, потому что иначе будет просто вызвана функция сложения и выведен ответ со знаком '-' или '+'. Для удобства рассмотрим два положительных числа (с двумя отрицательными числами всё аналогично, только в начале берется знак '-' и учитывается при выводе ответа).
