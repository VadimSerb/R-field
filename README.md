...
Если в начале была выбрана операция "вычитание", то заместо ```AdditionOperation``` будет вызвана функция ```void SubstractionOperation(char* a, char* b, char* c)```, которая работает следующим образом:
