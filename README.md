...
- Условное ограничение - выбрав операцию сложения, считается, что оба введенных числа являются положительными, то есть больше, либо равные нулю. Для сложения отрицательного числа с положительным, либо отрицательного с отрицательным используется операция "вычитание", для которой как раз таки не важен порядок ввода чисел.
   
- Ограничение является условным, потому что при желании можно добавить необходимую проверку на знак, чтобы можно было складывать положительное число с отрицательным, либо отрицательное с отрицательным при выборе операции "сложение".
...
```SubstractionOperation``` получает на вход три массива: first - целая часть первого числа, second - целая часть второго числа, и result, куда будет записал результат математического вычитания одного числа из другого. Далее ```SubstractionOperation``` вызывает функцию ```char ComparisonOperation(char* a, char* b)```, которая реализует беззнаковое сравнение first и second, возвращая символ 'T', если first >= second и 'F' в противном случае.
